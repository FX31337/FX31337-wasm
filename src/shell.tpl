...
<span id='controls'>
  ...
  <span><input type="button" value="Fullscreen" onclick="{{{ EXPORT_NAME }}}.requestFullscreen(document.getElementById('pointerLock').checked,
    ...
    <script type='text/javascript'>
      ...
      var {{{ EXPORT_NAME }}} = {
        ...
        setStatus: function(text) {
          if (!{{{ EXPORT_NAME }}}.setStatus.last) {{{ EXPORT_NAME }}}.setStatus.last = { time: Date.now(), text: '' };
          if (text === {{{ EXPORT_NAME }}}.setStatus.last.text) return;
          var m = text.match(/([^(]+)\((\d+(\.\d+)?)\/(\d+)\)/);
          var now = Date.now();
          if (m && now - {{{ EXPORT_NAME }}}.setStatus.last.time < 30) return; // if this is a progress update, skip it if too soon
          {{{ EXPORT_NAME }}}.setStatus.last.time = now;
          {{{ EXPORT_NAME }}}.setStatus.last.text = text;
          if (m) {
            text = m[1];
        ...
        monitorRunDependencies: function(left) {
          ...
          {{{ EXPORT_NAME }}}.setStatus(left ? 'Preparing... (' + (this.totalDependencies-left) + '/' + this.totalDependencies + ')' : 'All downloads complete.');
        }
      };
      {{{ EXPORT_NAME }}}.setStatus('Downloading...');
      window.onerror = function(event) {
        // TODO: do not warn on ok events like simulating an infinite loop or exitStatus
        {{{ EXPORT_NAME }}}.setStatus('Exception thrown, see JavaScript console');
        spinnerElement.style.display = 'none';
        {{{ EXPORT_NAME }}}.setStatus = function(text) {
